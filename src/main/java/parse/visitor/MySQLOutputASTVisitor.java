...
import parse.ast.expression.*;
import parse.ast.expression.arithmeic.ArithmeticBinaryOperatorExpression;
...
public final class MySQLOutputASTVisitor implements SQLASTVisitor {
    private static final Logger LOGGER = Logger.getLogger(MySQLOutputASTVisitor.class);
    ...
    @Override
    public void visit(Count node) {//TODO
        String functionName = node.getFunctionName();
        appendable.append(functionName).append('(');
        if (node.isDistinct()) {
            appendable.append("DISTINCT ");
    ...
    public void visit(Limit node) {
        ...
        if (!(offset instanceof ParamMarker) && !(size instanceof ParamMarker)) {
            ...
            if (offsetStr.equals("0") && sizeStr.equals("1")) {
                appendable.append(sizeStr);
                return;
            }
        }
    ...
    public void visit(DMLSelectStatement node) {
        ...
        if (limit != null) {
            ...
            limit.accept(this);
            try {
                String offsetStr = String.valueOf(limit.getOffset());
                String sizeStr = String.valueOf(limit.getSize());
                long offset = Long.parseLong(offsetStr);
                long size = Long.parseLong(sizeStr);
            } catch (Exception e) {
                LOGGER.error("limit change failed, exception: " + e);
            }
        }
