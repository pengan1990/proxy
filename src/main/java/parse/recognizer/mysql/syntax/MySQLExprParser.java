...
import parse.ast.expression.bit.*;
...
import parse.ast.expression.primary.function.comparison.Interval;
import parse.ast.expression.primary.function.groupby.*;
import parse.ast.expression.primary.function.info.CurrentUser;
import parse.ast.expression.primary.function.string.Char;
import parse.ast.expression.primary.function.string.Locate;
import parse.ast.expression.primary.function.string.Substring;
...
import parse.ast.expression.string.RegexpExpression;
import parse.ast.expression.string.SoundsLikeExpression;
import parse.ast.expression.type.CastBinaryExpression;
import parse.ast.expression.type.CollateExpression;
import parse.recognizer.mysql.MySQLFunctionManager;
import parse.recognizer.mysql.MySQLToken;
import parse.recognizer.mysql.lexer.MySQLLexer;
import parse.util.Pair;
import parse.ast.expression.arithmeic.*;
import parse.ast.expression.comparison.*;
import parse.ast.expression.logical.*;
import parse.ast.expression.misc.*;
import parse.ast.expression.primary.*;
import parse.ast.expression.primary.function.datetime.*;
...
public class MySQLExprParser extends MySQLParser {
    ...
    public Expression expression() throws SQLSyntaxErrorException {
        ...
        if (lexer.token() == MySQLToken.OP_ASSIGN) {
            lexer.nextToken();
     ...
     *
     * @throws SQLSyntaxErrorException expr:
     *                                 | NOT expr
     *                                 | ! expr
    ...
    private Expression logicalNotExpression() throws SQLSyntaxErrorException {
        int not = 0;
        for (; lexer.token() == MySQLToken.KW_NOT; ++not) {
            lexer.nextToken();
        }
            ...
            switch (lexer.token()) {
                case KW_NOT:
                    ...
                    switch (lexer.token()) {
                        case KW_BETWEEN:
                            lexer.nextToken();
                            Expression snd = comparisionExpression();//TODO 这里应该不需要用comparisionExpression
                        ...
                        case KW_IN:
                            if (lexer.nextToken() != MySQLToken.PUNC_LEFT_PAREN) {//走不通，然后又塞回去了？
                                lexer.addCacheToke(MySQLToken.KW_IN);
                                return fst;
                            }
                            Expression in = rightOprandOfIn();
                            //predicate: bit_expr [NOT] IN (subquery)
                            //         | bit_expr [NOT] IN (expr [, expr] ...)
                ...
                case KW_IS:
                    ...
                    //expr: boolean_primary IS [NOT] {TRUE | FALSE | UNKNOWN}
                ...
                case OP_EQUALS:
                    ...
                    //boolean_primary: boolean_primary comparison_operator {ALL | ANY} (subquery)
                ...
                default:
                    if (equalsIdentifier("SOUNDS") >= 0) {//predicate: bit_expr SOUNDS LIKE bit_expr
                        lexer.nextToken();
    ...
    private Expression rightOprandOfIn() throws SQLSyntaxErrorException {
        ...
        if (MySQLToken.KW_SELECT == lexer.token()) {
        ...
        if (consumed == null) {
            ...
            switch (lexer.token()) {
                ...
                case KW_BINARY:
                    lexer.nextToken();
                    expr = unaryOpExpression(null, null);
                    //simple_expr: BINARY simple_expr
    ...
    private Expression collateExpression(String consumed, String consumedUp) throws SQLSyntaxErrorException {
        for (Expression expr = userExpression(consumed, consumedUp); ; ) {
            if (lexer.token() == MySQLToken.KW_COLLATE) {
                ...
                //simple_expr: simple_expr COLLATE collation_name
    ...
    private Expression userExpression(String consumed, String consumedUp) throws SQLSyntaxErrorException {
        ...
        if (lexer.token() == MySQLToken.USR_VAR) {
    ...
    private Expression primaryExpression(final String consumed, String consumedUp) throws SQLSyntaxErrorException {
        ...
        switch (lexer.token()) {
            case PLACE_HOLDER: //simple_expr: param_marker
            ...
            case KW_CASE: //simple_expr: case_expr
                lexer.nextToken();
                return caseWhenExpression();

            case KW_INTERVAL://simple_expr: interval_expr
                lexer.nextToken();
                return intervalExpression();

            case KW_EXISTS://simple_expr: EXISTS (subquery)
                lexer.nextToken();
                match(MySQLToken.PUNC_LEFT_PAREN);
                tempExpr = subQuery();
                match(MySQLToken.PUNC_RIGHT_PAREN);
            ...
            case USR_VAR:
                ...
                if (lexer.nextToken() == MySQLToken.OP_ASSIGN) {
                    lexer.nextToken();
    ...
    private Expression startedFromIdentifier(final String consumed, String consumedUp) throws SQLSyntaxErrorException {
        ...
        switch (lexer.token()) {
            case PUNC_DOT:
                for (tempExpr = new Identifier(null, consumed, consumedUp).setCacheEvalRst(cacheEvalRst); lexer.token() == MySQLToken.PUNC_DOT; ) {
                    switch (lexer.nextToken()) {
                        case IDENTIFIER:
                            tempExpr =
            ...
            case PUNC_LEFT_PAREN:
                ...
                switch (functionManager.getParsingStrategy(consumedUp)) {
                    ...
                    case SUBSTRING:
                        ...
                        tempExprList.add(expression());
                        match(MySQLToken.PUNC_COMMA, MySQLToken.KW_FROM);
                        tempExprList.add(expression());
                        switch (lexer.token()) {
                            case PUNC_COMMA:
                            case KW_FOR:
                    ...
                    case TRIM:
                        ...
                        return new Trim(direction, tempExpr, tempExpr2).setCacheEvalRst(cacheEvalRst);
                    case AVG:
                        if (lexer.nextToken() == MySQLToken.KW_DISTINCT) {
                    ...
                    case GROUP_CONCAT:
                        ...
                        switch (lexer.token()) {
                            case KW_ORDER:
                                ...
                                tempExpr = expression();
                                if (lexer.token() == MySQLToken.KW_ASC) {
                                    lexer.nextToken();
                                } else if (lexer.token() == MySQLToken.KW_DESC) {
                                    isDesc = true;
                                    lexer.nextToken();
                                }
                                for (appendedColumnNames = new LinkedList<Expression>(); lexer.token() == MySQLToken.PUNC_COMMA; ) {
                                    lexer.nextToken();
                                    appendedColumnNames.add(expression());
                                }
                                if (lexer.token() != MySQLToken.KW_SEPARATOR) {
                                    break;
                                }
                            case KW_SEPARATOR:
                                lexer.nextToken();
                                ...
                                match(MySQLToken.LITERAL_CHARS);
                                break;
                        }
                        match(MySQLToken.PUNC_RIGHT_PAREN);
    ...
    private Pair<String, Pair<Expression, Expression>> type4specialFunc() throws SQLSyntaxErrorException {
        ...
        switch (lexer.token()) {
            ...
            case KW_UNSIGNED:
                typeName = MySQLToken.keyWordToString(lexer.token());
                if (lexer.nextToken() == MySQLToken.KW_INTEGER) {
                    lexer.nextToken();
                }
            ...
            case IDENTIFIER:
                ...
                if ("SIGNED".equals(typeName)) {
                    if (lexer.token() == MySQLToken.KW_INTEGER) {
                        lexer.nextToken();
                    }
    ...
    private Expression intervalExpression() throws SQLSyntaxErrorException {
        ...
        if (lexer.token() == MySQLToken.PUNC_LEFT_PAREN) {
            if (lexer.nextToken() == MySQLToken.KW_SELECT) {
                fstExpr = subQuery();
                match(MySQLToken.PUNC_RIGHT_PAREN);
            } else {
                fstExpr = expression();
    ...
    private Expression caseWhenExpression() throws SQLSyntaxErrorException {
        ...
        for (; lexer.token() == MySQLToken.KW_WHEN; ) {
            lexer.nextToken();
