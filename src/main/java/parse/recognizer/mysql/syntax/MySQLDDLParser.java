...
import parse.ast.expression.primary.literal.Literal;
...
public class MySQLDDLParser extends MySQLParser {
        ...
        for (int i = 0; lexer.token() != MySQLToken.EOF; ++i) {
            ...
            switch (lexer.token()) {
                ...
                case KW_RENAME:
                    ...
                    if (lexer.nextToken() == MySQLToken.KW_TO) {
                        lexer.nextToken();
                    }
                    id = identifier();
                    stmt.setRenameTo(id);
    ...
    private IndexDefinition indexDefinition() throws SQLSyntaxErrorException {
        ...
        match(MySQLToken.PUNC_LEFT_PAREN);
        for (int i = 0; lexer.token() != MySQLToken.PUNC_RIGHT_PAREN; ++i) {
            if (i > 0) match(MySQLToken.PUNC_COMMA);
    ...
    private DataType dataType() throws SQLSyntaxErrorException {
        ...
        switch (lexer.token()) {
            case KW_TINYINT:
                ...
                if (lexer.token() == MySQLToken.KW_UNSIGNED) {
                    unsigned = true;
                    lexer.nextToken();
                }
                if (lexer.token() == MySQLToken.KW_ZEROFILL) {
                    zerofill = true;
                    lexer.nextToken();
                }
                break typeName;
            ...
            case KW_TINYTEXT:
                ...
                if (lexer.nextToken() == MySQLToken.KW_BINARY) {
                    lexer.nextToken();
                    binary = true;
                }
                if (lexer.token() == MySQLToken.KW_CHARACTER) {
    ...
    private ColumnDefinition columnDefinition() throws SQLSyntaxErrorException {
        ...
        if (lexer.token() == MySQLToken.KW_NOT) {
            ...
            notNull = true;
        } else if (lexer.token() == MySQLToken.LITERAL_NULL) {
            lexer.nextToken();
        }
    ...
    private boolean tableOption(TableOptions options) throws SQLSyntaxErrorException {
        ...
        switch (lexer.token()) {
            ...
            case KW_UNION:
                ...
                for (int j = 0; lexer.token() != MySQLToken.PUNC_RIGHT_PAREN; ++j) {
                    if (j > 0) match(MySQLToken.PUNC_COMMA);
                    id = identifier();
                    union.add(id);
                }
                match(MySQLToken.PUNC_RIGHT_PAREN);
