...
import parse.ast.expression.primary.SysVarPrimary;
import parse.ast.expression.primary.UsrDefVarPrimary;
...
public class MySQLDALParser extends MySQLParser {
    ...
    static {
        ...
        specialIdentifiers.put("AUTOCOMMIT", SpecialIdentifier.AUTOCOMMIT);
    }
    ...
    public DALShowStatement show() throws SQLSyntaxErrorException {
        ...
        switch (lexer.token()) {
            ...
            case KW_TABLE:
                ...
                tempId = null;
                if (lexer.token() == MySQLToken.KW_FROM || lexer.token() == MySQLToken.KW_IN) {
                    lexer.nextToken();
                    tempId = identifier();
                }
                switch (lexer.token()) {
    ...
    private ShowProfile showProfile() throws SQLSyntaxErrorException {
        ...
        } else if (lexer.token() == MySQLToken.PUNC_COMMA) {
        ...
        if (lexer.token() == MySQLToken.KW_FOR) {
            lexer.nextToken();
    ...
    private String getStringValue() throws SQLSyntaxErrorException {
        ...
        switch (lexer.token()) {
            case LITERAL_NUM_PURE_DIGIT:
                StringBuilder autocommit = new StringBuilder();
                for (int index = lexer.getCurrentIndex()  - lexer.getSizeCache();
                     index < lexer.getCurrentIndex(); index ++) {
                    autocommit.append(lexer.getSQL()[index]);
                }
                return autocommit.toString().trim();
            case IDENTIFIER:
    ...
    public SQLStatement set() throws SQLSyntaxErrorException {
        ...
        if (lexer.token() == MySQLToken.IDENTIFIER
            ...
            return new DALSetNamesStatement(charsetName, collationName);
                ...
                SpecialIdentifier.AUTOCOMMIT == specialIdentifiers.get(lexer.stringValueUppercase())) {
            if (lexer.nextToken() == MySQLToken.OP_EQUALS) {
                lexer.nextToken();
            }
            String autocommit = getStringValue();
            lexer.nextToken();
            return new DALSetAutocommitStatement(autocommit);
        } else if (lexer.token() == MySQLToken.KW_CHARACTER) {
            lexer.nextToken();
            match(MySQLToken.KW_SET);
    ...
    private MTSSetTransactionStatement setMTSSetTransactionStatement(VariableScope scope)
        ...
        switch (lexer.token()) {
            case KW_READ:
                ...
                throw err("unknown isolation readFromChannel level: " + lexer.stringValue());
            case IDENTIFIER:
                si = specialIdentifiers.get(lexer.stringValueUppercase());
                if (si != null) {
                    switch (si) {
